                    ...
                    { (sessionUser._id === tuneUp.host) && 
                        tuneUp.pendingConnections.map(requestingUserId => (
                            <div key={requestingUserId}>
                                <span>{users[requestingUserId]?.firstName} wants to join</span>
                                <button onClick={() => handleAcceptRequest(requestingUserId)}>Accept</button>
                                <button onClick={() => handleDenyRequest(requestingUserId)}>Deny</button>
                            </div>
                        ))
                    }
